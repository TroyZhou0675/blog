...
$$M_+(t) = M_x(t) + iM_y(t)$$
用复数改写方程
$$\frac{dM_+}{dt} = i\gamma B_1 M_z -i\Delta \omega M_+----Eq1.$$
$$\frac{dM_z}{dt}=i\frac {\gamma}{2}(\overline{B_1}M_+-B_1\overline{M_+}) = -\gamma Im(\overline{B_1}M_+)----Eq2.$$
**在翻转角度较小时，采用近似：** $M_z = M_0$
$$Eq1.: \frac{dM_+}{dt} = i\gamma B_1 M_0 -i\Delta \omega M_+$$
解得：
$$M_+(\Delta \omega) = i\gamma M_0e^{-i\Delta \omega t}\int_{0}^{t} B_1(t)e^{-i \Delta \omega t}dt$$
而右边这个积分式，正好是$B_1(t)$经过傅里叶变换后的形式
所以得到：
